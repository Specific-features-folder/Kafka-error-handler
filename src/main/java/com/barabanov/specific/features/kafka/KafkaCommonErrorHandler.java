...
public class KafkaCommonErrorHandler implements CommonErrorHandler {
     ...
     * <p>
     * В логике сервиса должна быть предусмотрена обработка ошибки при работе с батчем. Например, обрабатывать батч по одному сообщение в случае возникновения ошибки.
    ...
    public <K, V> ConsumerRecords<K, V> handleBatchAndReturnRemaining(Exception thrownException,
        ...
        String logMsg = "При обработке батча сообщений из kafka из партиций {} произошла ошибка. ";
        if (databaseHelper.isDatabaseAvailable())
            logMsg += "При обработки ошибки подключение к БД присутствует.";
        else
            logMsg += "При обработки ошибки подключение к БД отсутствует!";

        log.error(logMsg, data.partitions());
